...
if($dbresult)
{
	if($dbresult->RecordCount()>0)
	{
		...
		while($row = $dbresult->FetchRow())
		{
			...
			$query2 = "SELECT equa, equb, scorea, scoreb FROM ".cms_db_prefix()."module_ping_poules_rencontres WHERE saison = ? AND idepreuve = ? AND date_event <= NOW() AND (equa = ? OR equb = ?) ORDER BY date_event DESC LIMIT 1";
...
//echo $this->ProcessTemplate('xibo_xml_details_rencontre.tpl');
