...
$query = "SELECT sp.epreuve,p.coefchamp,p.codechamp FROM ".cms_db_prefix()."module_ping_parties_spid AS sp, ".cms_db_prefix()."module_ping_parties AS p WHERE sp.licence = p.licence AND sp.nom = p.advnompre AND sp.date_event = p.date_event AND sp.saison = ? AND ((sp.pointres !=p.pointres) OR (sp.numjourn !=p.numjourn) OR (sp.coeff != p.coefchamp)) ORDER BY sp.id ASC";
...
$i = 0;
	if($dbresult && $dbresult->RecordCount()>0)
	{
		while($row = $dbresult->FetchRow())
		{
			...
			$coefchamp = $row['coefchamp'];
			$codechamp = $row['codechamp'];
			$epreuve = $row['epreuve'];
			
			//on va chercher si les données ci-dessus sont bien renseignées ds la table type_competitions
			//si non, on fait un update
			$query2 = "SELECT name, code_compet, coefficient FROM ".cms_db_prefix()."module_ping_type_competitions WHERE name = ?";
			$dbresult2 = $db->Execute($query2, array($epreuve));
