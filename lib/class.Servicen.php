...
class Servicen
{
   ...
   public function GetEncryptedTimestamp()
   {
	...
	//$serial = $ping->GetPreference('serie');
	//$serial = $this->serials[mt_rand(0, count($this->serials) - 1)];
 
        $this->getData('http://www.fftt.com/mobile/pxml/xml_initialisation.php', array('serie' => $this->serie, 'tm' => $this->tm, 'tmc' => $this->tmc, 'id' =>$this->id));
        $event = self::getObject($this->getData('http://www.fftt.com/mobile/pxml/evenement/xml_evenement.php'), 'evenement');
        if (!empty($event['logo'])) {
    ...
    public function getClub($numero)
    {
        return $this->getCachedData("club_{$numero}", 3600*24*2, function($service) use ($numero) {
            return Service::getObject($service->getData('http://www.fftt.com/mobile/pxml/xml_club_detail.php', array('serie' => $this->serie, 'tm' => $this->tm, 'tmc' => $this->tmc, 'id' => $this->id,'club' => $numero)),array('serie','tm','tmc','id','club'));
        });
    }
    ...
    public function getDivisions($organisme, $epreuve)
    {
        return $this->getCachedData("divisions_{$organisme}_{$epreuve}", 3600*24*30, function($service) use ($organisme, $epreuve) {
            return Service::getCollection($service->getData('http://www.fftt.com/mobile/pxml/xml_division.php', array('organisme' => $organisme, 'epreuve' => $epreuve, )), 'division');
        });
    }
