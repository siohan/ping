...
$licence = (isset($params['licence']) ? $params['licence']: '292271');//'5310809';
/*
...
$query = "SELECT * ,licence, CONCAT_WS(' ',nom, prenom) AS player FROM  ".cms_db_prefix()."module_ping_joueurs WHERE actif = 1 ORDER BY nom ASC";//"";
...
/**/
$result = array();
$query2 = "SELECT CONCAT_WS(' ', j.nom, j.prenom) AS joueur,p.licence,SUM(p.pointres) as sumpoints,count(p.vd) - SUM(p.vd) AS defaites, SUM(p.vd) AS victoires,count(p.vd) AS matchs, MONTH(p.date_event) AS mois FROM ".cms_db_prefix()."module_ping_parties AS p, ".cms_db_prefix()."module_ping_joueurs AS j WHERE j.licence = p.licence AND p.saison = ? AND p.licence = ? ";//GROUP BY mois ORDER BY YEAR(p.date_event) ASC,mois";
$parms['saison'] = $saison;
