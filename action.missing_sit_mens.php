...
$result= array ();
//on fait une première requete pour connaitre ceux dont la situation mensuelle est dûment completée
$query1 = "SELECT j.licence, CONCAT_WS(' ',j.nom, j.prenom) AS joueur FROM ".cms_db_prefix()."module_ping_joueurs AS j LEFT JOIN  ".cms_db_prefix()."module_ping_sit_mens AS st  ON st.licence = j.licence AND st.mois = ?  AND st.annee = ? AND actif = '1'";
$dbresult1 = $db->Execute($query1,array($mois_courant,$annee_courante));
echo $query1;

//on peut préparer la deuxième requete avec le NOT IN
