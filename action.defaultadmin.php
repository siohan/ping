...
if(isset($params['__activetab']) && !empty($params['__activetab']))
  ...
  $tab = 'rencontres';
	...
	if (true == $this->GetPreference('epreuv_tab'))
	{
		echo $this->SetTabHeader('compets', 'Epreuves' , ('compets' == $tab)?true:false);
	}
	...
	if (true == $this->GetPreference('contacts_tab'))
	{	
	...
	if($this->CheckPermission('Ping Set Prefs'))
	{
		...
		if (true == $this->GetPreference('compte_tab'))
		{
...
echo $this->StartTabContent();
	...
	echo $this->StartTab('equipes' , $params);//les équipes
   	...
   	echo $this->StartTab('brulage' , $params);//le brûlage
    	include(dirname(__FILE__).'/action.brulage_par_eq.php');
   	echo $this->EndTab();
