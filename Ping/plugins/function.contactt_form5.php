...
#
# (c)2004-2013by Ted Kulp (wishy@users.sf.net)
#
...
function smarty_cms_function_contactt_form5($params, &$template) { 

	$smarty = $template->smarty;
	// init
	$date_serveur = date("Y-m-d H:m:s"); // date du jour
	//Will be Eq to null or to Captcha instance.
    $captcha = getCaptchaInstancet($params);
	//Will be Eq to null or to CmsMailer instance.
	$cmsmailer = getCmsMailerInstancet($params);
// -------------------------- langue ------------------------------
...
//Messages erreur			
	$errors=$name=$email=$subject=$message=$telephone = '';
	$bodymessage = '';
	...
	if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if (!empty($_POST['telephone'])) $telephone = ($_POST['telephone']); //+telephone
		if (!empty($_POST['email'])) $email = cfsanitizet($_POST['email']);
...
//Mail headers
		...
		elseif (!validEmailt($email)) $errors .= "\t\t<li>" . $lang[7] . "</li>\n";
...
// Start Form  
	...
	$html_telephone = htmlspecialchars($telephone); 
    ...
    <form action="{$base_url}" method="post" name="contactForm">
		<label for="name" title="{$lang[18]}" $styleLabel><mark>*</mark> {$lang[19]} :</label>
		<input type="text" name="name" id="name" placeholder="{$lang[19]}" title="{$lang[18]}" value="{$html_name}" size="50" $styleInput  autofocus required >
		
		<label for="telephone" title="{$lang[31]}" $styleLabel>&nbsp; <span style="font-size: x-small;">{$lang[32]}</span></label>
		<input type="text" name="telephone" id="telephone" placeholder="00 00 00 00 00 00" title="{$lang[31]}" value="$html_telephone" size="50" $styleInput>
		
		<label for="email" title="{$lang[20]}" $styleLabel><mark>*</mark> {$lang[21]} :</label>
		<input type="email" name="email" id="email" placeholder="{$lang[21]}" title="{$lang[20]}"  value="{$html_email}" size="50" $styleInput required >

		<label for="subject" title="{$lang[22]}" $styleLabel><mark>*</mark> {$lang[23]} :</label>	
		<input type="text" name="subject" id="subject" placeholder="{$lang[23]}" title="{$lang[22]}"  value="{$html_subject}" size="50" $styleInput required >
			
		<label for="message" title="{$lang[24]}" $styleLabel><mark>*</mark> {$lang[25]} :</label>
		<textarea name="message" id="message" placeholder="{$lang[25]}" title="{$lang[24]}" cols="40" rows="10" {$styleInput} required >{$html_message}</textarea>
...
// Start captcha
	if (null != $captcha) {
		if(!$recaptcha) {
			echo <<<HTML
				<label for="recaptcha_response_field" title="{$lang[26]}" {$styleLabel}><mark>*</mark> {$lang[26]} : </label><br />
				<input type="text" name="recaptcha_response_field"  id="recaptcha_response_field" placeholder="{$lang[27]}" title=" {$lang[26]}" value="" size="20" {$captchaStyle} required />
HTML;
		}
			echo <<<HTML
...
// End captcha
	...
	echo <<<HTML
		...
		</form> <br /><small><mark>* {$lang[29]}</mark></small>
HTML;
...
function smarty_cms_help_function_contactt_form5() {
	?>
  <h2>NOTE : Ce plugin est obsol&egrave;te</h2>
	...
	<p>Affiche un formulaire de contact avec zone t&eacute;l&eacute;phone. Celui-ci peut &ecirc;tre utilis&eacute; par vos visiteurs pour envoyer un mail &agrave; une adresse sp&eacute;cifi&eacute;e.</p>
	<h3>Comment l'utiliser ?</h3>
	<p>Ins&eacute;rer la balise dans votre page comme : <code>{contactt_form5 email='yourname@yourdomain.com'}</code><br />
	...
	<ul>
		...
		<li><em>(option)</em> subject_get_var - Cha&icirc;ne de caract&egrave;res pour sp&eacute;cifier le sujet par d&eacute;faut.
               ...
               <p>Exemple:</p>
               <pre>{contactt_form5 email='votre_nom@votredomaine.com' subject_get_var='subject'}</pre>
	...
	<ul>
		...
		<li>+ Ajout pour HTML 5 des balises HTML5 placeholder + required</li>
		<li>Si config avec CmsMailer, le reply-to sera définit à l'adresse fournie dans le formulaire</li>
		<li>le multi destinataire séparé par des virgules ne fonctionnait pas en mode CmsMailer</li>
		<li>Mise à niveau CMSms 1.11.x + mise &agrave; jour pour ajout zone téléphone.</li>
...
function smarty_cms_about_function_contactt_form5() {
	?>
	...
	Change History:<br/>
		...
		<li>1.6.1 : 11/03/2012 ajout couleur rouge et zone téléphone +  Ajout pour HTML 5 des balises HTML5 
		+ placeholder + required (by jce76350)</li>
		<li>1.6.1-5 : Si config avec CmsMailer, le reply-to sera définit à l'adresse fournie dans le formulaire</li>
		<li>1.6.2-5 : le multi destinataire séparé par des virgules ne fonctionnait pas en mode CmsMailer(by bess)</li>
		<li>1.8.4T-5 : Mise à niveau CMSms 1.11.x + mise &agrave; jour pour ajout zone téléphone (by jce76350)</li>	
		<li>1.8.5T-5 : Correction potentiel XSS sur form action=.... (by bess)</li>
		<li>1.8.6T-5 : Nettoyage des commentaires (by jce76350)</li>
		<li>1.9.0T-5 : Ajout détection langue FR et EN (by jissey)</li>
		<li>1.9.1T-5 : Ajout détection langue ES (by jissey)</li>
		<li>1.9.2T-5 : Suppresion des x-moz-errormessage= dans les version HTML5 Suppresion de utf8_decode sur utf8_decode($subject) (by JCE)</li>
        </ul>
	</p>
	<?php
}
...
function getCaptchaInstancet($params){
...
function getCmsMailerInstancet($params){
...
function cfsanitizet($content){
	return str_replace(array("\r", "\n"), "", trim($content));
}

function validEmailt($email) {
...
function getTranslationt($langue) {
		switch ($langue) {
		case "en" :
		...
		$lang[30] = "phone number : ";
		$lang[31] = "Please, enter your phone number";
		$lang[32] = "Your phone number : ";
		break;
		case "es" :
		...
		$lang[30] = "número de teléfono : ";
		$lang[31] = "Por favor, introduzca su número de teléfono";
		$lang[32] = "Su número de teléfono : ";
		break;
		//-------------------- French by default --------------------------
		default : 
		...
		$lang[30] = "telephone : ";
		$lang[31] = "Veuillez saisir votre t&eacute;l&eacute;phone";
		$lang[32] = "Votre t&eacute;l&eacute;phone : ";
		}
return $lang;
}
# fin
