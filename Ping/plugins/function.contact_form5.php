...
#
#  by Ted Kulp (wishy@users.sf.net)
#
...
//function smarty_function_contact_form5($params, &$smarty) {
...
//Messages erreur			
	...
	if($_SERVER['REQUEST_METHOD']=='POST') {
		...
		if (!empty($_POST['email'])) $email = cfsanitize5($_POST['email']);
		...
		if (null != $captcha) 
{
			...
			if (!empty($_POST['recaptcha_response_field'])) { $captcha_resp = $_POST['recaptcha_response_field']; }
...
//Mail headers
		...
		elseif (!validEmail5($email)) $errors .= "\t\t<li>" . $lang[7] . "</li>\n";
...
HTML;
			...
			if(null != $cmsmailer) {
				  $tos = explode(',', $to); // add bess  09/09/2012
				  foreach($tos as $to){
				  	$cmsmailer->AddAddress($to,$to);
				  }
				  //$cmsmailer->AddAddress($to,$to); //remplacer par ci-dessus 09/09/2012
				  ...
				  $cmsmailer->AddReplyTo($email); // Add 1.8.3(idem 1.6.1 le reply-to  définit à l'adresse formulaire)		  
...
function smarty_cms_help_function_contact_form5() {
	?>
  <h2>NOTE : Ce plugin est obsol&egrave;te</h2>
	...
	<p>Affiche un formulaire de contact pour HTML 5. Celui-ci peut &ecirc;tre utilis&eacute; par vos visiteurs pour envoyer un mail &agrave; une adresse sp&eacute;cifi&eacute;e.</p>
	<h3>Comment l'utiliser ?</h3>
	<p>Ins&eacute;rer la balise dans votre page comme : <code>{contact_form5 email='yourname@yourdomain.com'}</code><br />
	...
	<ul>
		...
		<li><em>(option)</em> subject_get_var - Cha&icirc;ne de caract&egrave;res pour sp&eacute;cifier le sujet par d&eacute;faut.
               ...
               <p>Exemple:</p>
               <pre>{contact_form5 email='votre_nom@votredomaine.com' subject_get_var='subject'}</pre>
	...
	<ul>
		...
		<li>Pour CmsMadesimple 1.11.x - devient smarty_function_contact_form5 + Ajout pour HTML 5 des balises HTML5 MARK + placeholder + required</li>
...
function smarty_cms_about_function_contact_form5() {
	?>
	...
	Change History:<br/>
		...
		<li>1.8 : CmsMadesimple 1.11.x remplace  smarty_cms_function_contact_form par smarty_function_contact_form ( cause version Smarty 3.x sur CMSms 1.11)</li>
		<li>1.8.1-5 : 11/03/2012 CmsMadesimple 1.11.x - devient smarty_function_contact_form5 + Ajout pour HTML 5 des balises HTML5 MARK + placeholder + required (by jce76350)</li>
		<li>1.8.2-5 : 16/04/2012 remplace  ( $params, &$smarty ) par  ( $params, &$template ) Et ajout de  $smarty = $template->smarty; (by jce76350)</li>
		<li>1.8.3-5 : 05/05/2012 CmsMadesimple 1.11.x - remplace smarty_function_contact_form par smarty_cms_function_contact_form POUR ne PAS mettre en cache (by jce76350) + Si config avec CmsMailer, le reply-to sera définit à l'adresse fournie dans le formulaire (by bess)</li>	
		<li>1.8.4-5 : le multi destinataire séparé par des virgules ne fonctionnait pas en mode CmsMailer(by bess)</li>	
		<li>1.8.5-5 : Correction potentiel XSS sur form action=.... (by bess)</li>
		<li>1.8.6-5 : Nettoyage des commentaires (by jce76350)</li>
		<li>1.9.0-5 : Ajout détection langue FR et EN (by jissey)</li>
		<li>1.9.1-5 : Ajout détection langue ES (by jissey)</li>
		<li>1.9.2-5 : Suppresion des x-moz-errormessage= dans les version HTML5 Suppresion de utf8_decode sur utf8_decode($subject) (by JCE)</li>
        </ul>
	</p>
	<?php
}
...
function getCaptchaInstance5($params){
...
function getCmsMailerInstance5($params){
...
function cfsanitize5($content){
	return str_replace(array("\r", "\n"), "", trim($content));
}

function validEmail5($email) {
