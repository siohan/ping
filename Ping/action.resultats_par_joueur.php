...
$datelist[$this->Lang('alldates')] = 'Toutes les dates';	
$equipelist[$this->Lang('allequipes')] = 'Toutes les équipes';
$playerslist[$this->Lang('allplayers')] = 'Tous les joueurs';
$typeCompet = array();
$typeCompet[$this->Lang('allcompet')] = 'Toutes les compétitions';
...
$result= array ();
$query = "SELECT CONCAT_WS(' ', j.nom, j.prenom) AS joueur, pts.id, pts.vd, pts.numjourn,pts.date_event, pts.advnompre,pts.pointres, pts.advclaof  FROM ".cms_db_prefix()."module_ping_parties_spid AS pts , ".cms_db_prefix()."module_ping_joueurs AS j WHERE pts.licence = j.licence AND pts.saison = ?";
...
echo $this->ProcessTemplate('fe_indivscores.tpl');
